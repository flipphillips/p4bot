...
- Secrets: passwords and config may be provided via `P4TICKETS` and `P4CONFIG` files (see env `P4TICKETS` and `P4CONFIG`). Do not hardcode credentials. For local development the compose files mount `./app` -> `/srv/p4bot/app` and may mount host `~/.p4config` or `~/.p4tickets` into the container.
- Config: prefer using `P4CONFIG` (or environment variables `P4PORT`, `P4USER`, `P4TICKETS`) for runtime Perforce settings instead of a repository `scripts` directory.
...
- P4Python usage: use `_p4_instance()` in `server.py` to pick up perforce settings from `P4CONFIG` or environment variables (`P4PORT`, `P4USER`, `P4TICKETS`). Always `connect()` and `disconnect()`; use `run(cmd, *args)` and handle dict/list outputs by converting dicts to `key: value` lines (see `_run_p4_command`).
